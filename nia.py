...
class NIA():
    ...
    def open(self):
        ...
        self.device = self.device_descriptor.get_device()
        if not self.device:
            print("Failed to open NIA device. Cable isn't plugged in")
            return False
        try:
...
class NiaData():
    ...
    def get_data(self):
        ...
        Raw_Data = array([])
        try:
            for a in range(int(self.Points)):
            ...
            print("Failed to access NIA device: Access Denied")
            print("If you're on GNU/Linux, see README Troubleshooting section for details")
